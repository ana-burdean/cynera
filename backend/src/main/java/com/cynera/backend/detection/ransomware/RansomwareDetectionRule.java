...
public class RansomwareDetectionRule implements DetectionRule {
    ...
    public Optional<DetectionMatch> evaluate(SecurityEvent event) {
        ...
        FileBehaviorTracker.FileBehaviorSnapshot snapshot = fileBehaviorTracker.getSnapshot(processId);

        boolean massModification = isMassModification(snapshot);
        boolean suspiciousExtension = hasSuspiciousExtension(event.getFilePath());
        ...
        if (massModification) {
            return Optional.of(new DetectionMatch(
                    Severity.HIGH,
                    String.format("Mass file modification detected: %d files in %.1f sec (process: %s)",
        ...
        if (suspiciousExtension) {
            return Optional.of(new DetectionMatch(
                    Severity.MEDIUM,
                    String.format("Suspicious file extension detected: %s (process: %s)",
                            event.getFilePath(), event.getProcessName())
            ));
        }
    ...
    private boolean isFileEvent(SecurityEvent event) {
        return event.getEventType() != null && event.getEventType().startsWith("FILE_")
                && event.getFilePath() != null
                && event.getFileAction() != null;
    }

    private String buildProcessId(SecurityEvent event) {
        return event.getHostname() + ":" + event.getProcessName() + ":" + event.getParentProcessName();
    }

    private FileBehaviorTracker.FileAction mapFileAction(String fileAction) {
        if (fileAction == null) return FileBehaviorTracker.FileAction.MODIFIED;
    ...
    private boolean isMassModification(FileBehaviorTracker.FileBehaviorSnapshot snapshot) {
        return snapshot.getTotalModifications() >= MASS_MODIFICATION_THRESHOLD
                && snapshot.getModificationsPerSecond() >= MASS_MODIFICATION_RATE;
    }

    private boolean hasSuspiciousExtension(String filePath) {
        if (filePath == null) return false;
        String lowerPath = filePath.toLowerCase(Locale.ROOT);
        return SUSPICIOUS_EXTENSIONS.stream().anyMatch(lowerPath::endsWith);
    }

    private String buildDescription(FileBehaviorTracker.FileBehaviorSnapshot snapshot,
                                    boolean suspiciousExt) {
        StringBuilder sb = new StringBuilder("Ransomware-like activity detected: ");
        sb.append(snapshot.getTotalModifications()).append(" file modifications in ");
        sb.append(String.format("%.1f", snapshot.getModificationsPerSecond())).append(" sec");

        if (suspiciousExt) sb.append(" with suspicious extensions");

        sb.append(" (process: ").append(snapshot.getProcessId()).append(")");
        return sb.toString();
    }
}